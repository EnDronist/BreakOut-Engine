	...
	hInst(hInst)
{
	...
	RECT wr;
	wr.left = (GetSystemMetrics(SM_CXSCREEN) / 2) - (Settings::ScreenWidth() / 2);
	wr.right = wr.left + Settings::ScreenWidth();
	wr.top = (GetSystemMetrics(SM_CYSCREEN) / 2) - (Settings::ScreenHeight() / 2);
	wr.bottom = wr.top + Settings::ScreenHeight();
	size = { wr.right - wr.left, wr.bottom - wr.top };
	AdjustWindowRect(&wr, WS_CAPTION | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SYSMENU, FALSE);
	hWnd = CreateWindow(wndClassName, wnd_name.c_str(),
		WS_CAPTION | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SYSMENU,
		wr.left, wr.top, wr.right - wr.left, wr.bottom - wr.top,
		nullptr, nullptr, hInst, this);
	// throw exception if something went terribly wrong
	if (hWnd == nullptr) {
		throw Exception(L"Failed to get valid window handle.", _CRT_WIDE(__FILE__), __LINE__);
	}
	// show and update
...
LRESULT MainWindow::HandleMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_MOUSEMOVE: {
		int x = LOWORD(lParam);
		int y = HIWORD(lParam);
		if (x > 0 && x < Settings::ScreenWidth() && y > 0 && y < Settings::ScreenHeight()) {
			...
			if (wParam & (MK_LBUTTON | MK_RBUTTON)) {
				x = std::max(0, x);
				x = std::min(int(Settings::ScreenWidth()) - 1, x);
				y = std::max(0, y);
				y = std::min(int(Settings::ScreenHeight()) - 1, y);
				mouse.OnMouseMove(x, y);
			}
			else {
				ReleaseCapture();
